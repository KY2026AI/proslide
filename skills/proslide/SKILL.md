...
name: proslide
description: |
  ProSlide - 智能幻灯片生成器（HTML预览→四层防打回质检→高清还原版或混合可编辑版PPTX）。当用户提到"PPT"、"幻灯片"、"汇报"、"presentation"、"HTML预览"、"生成页面"、"优化PPT"、"做成PPT"、"可编辑PPT"时必须使用本技能，即使用户没有显式说出 ProSlide。核心流程是多轮确认：参数确认 → 诊断/延伸确认 → HTML预览 → 四层防打回质检 → 用户确认（含讲稿确认）→ 选择导出模式 → 导出PPTX。
---
...
7. **导出模式确认**：只有在用户确认 HTML 无需调整、且讲稿需求已确认后，才允许询问导出模式。必须让用户选择“高清还原版 / 混合可编辑版 / 两版都要”，禁止默认替用户选择。
8. **导出确认**：只有在用户明确选择导出模式后，才允许调用 `proslide-export` 导出 PPTX。
...
| `HTML_PREVIEW_READY` | 生成 HTML 预览、执行四层防打回质检、提供路径/截图 | 导出 PPTX | 质检通过，且用户确认 HTML 无需调整，并确认讲稿需求 |
| `SPEAKER_NOTES_CONFIRM` | 确认讲稿语言和时长，生成讲稿 | 导出 PPTX | 讲稿生成或用户选择不需要 |
| `EXPORT_MODE_CONFIRM` | 说明两种导出模式并等待用户选择 | 未确认模式就导出 PPTX | 用户选择高清还原版、混合可编辑版或两版都要 |
| `EXPORT_READY` | 按已选模式调用 `proslide-export` 导出 PPTX | 跳过对应模式的导出 QA | 导出完成并验证 |
...
在 HTML 和讲稿需求均确认后，固定向用户提供以下选项：

```md
请选择 PPTX 导出方式：

1. **高清还原版**：将每页 HTML 以高清截图铺满 PPT，视觉还原度最高；页面内容作为整页图片，不能逐项编辑。
2. **混合可编辑版**：文字、表格、基础形状、色块、线条、进度条等优先转换为 PowerPoint 原生可编辑元素；复杂图表、SVG 图标或特殊视觉效果使用矢量图或高清图片。整体尽量接近 HTML，但可能存在轻微字体、间距或换行差异。
3. **两版都要**：同时输出高清还原版和混合可编辑版，分别用于正式展示和后续修改。
```

必须等用户明确选择后再进入 Step 12。

## Step 12：导出 PPTX

用户确认导出模式后，调用 `proslide-export` 按所选模式执行。高清还原版优先使用现有截图脚本；混合可编辑版按原生元素映射规则重建；若导出或渲染失败，再读取 `references/failure-recovery.md`。
...
- 导出模式已由用户明确选择。
- 高清还原版使用 `.slide` 元素级高清截图，不得用整页截图后裁切替代。
- 混合可编辑版不得用“整页截图嵌入 SVG”冒充可编辑内容；文字与基础形状必须是真正的 PowerPoint 原生对象。
- 导出的 PPTX 完成后必须做页数、尺寸、内容完整性、视觉溢出和对应模式的质量检查。
...
- 导出 PPTX 时：调用 `proslide-export`；高清还原版优先使用仓库 `src/export.py` 或安装环境提供的等效截图脚本，混合可编辑版按其原生元素映射规范执行
...
- ❌ 将整页位图包进 SVG 后宣称为“混合可编辑版”
- ❌ 混合可编辑版把所有内容都栅格化，导致文字和基础形状无法编辑
