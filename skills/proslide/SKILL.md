...
在生成 HTML 前，先根据报告类型、页数、素材密度规划页面结构。生成 HTML 前读取 `references/layout-rules.md`；当内容包含多种关系、连续页面容易卡片化、或需要在并列/对比/递进/分解/时间计划/分析框架/数据证据之间做版式取舍时，再读取 `references/layout-pattern-library.md`。只加载当前阶段需要的版式规则。
...
- 版式选择必须从内容关系出发，而不是从模板外观出发；不要建立或套用截图模板库，不要为了“像某页模板”牺牲 ProSlide 的统一页眉、观点条、配色和可读性。
- 视觉元素必须语义化：图标用 SVG/图片，不用文字、数字、emoji 伪装；明显留白按版式规则重排。
- 低信息量卡片禁止被 `flex: 1`、大 `min-height`、固定高度或等高网格强行撑满页面；卡片容器必须匹配信息量，明显空卡必须改成紧凑条形/流程/信号组，或补充有语义的判断条件、适用边界、案例启发。
- 输出文案必须采用正式汇报语气，同时减少“AI味”和行业黑话：避免空泛口号、抽象词堆叠、未解释术语和过度口语化表达；优先使用目标受众能理解的业务对象、具体动作和实际变化。
...
1. **L1 页面硬伤检查**：像语法检查一样排除硬错误。检查标题是否完整、字号是否低于下限、logo/顶部横线/核心观点条是否存在、图片是否完整显示、是否有文字溢出/遮挡/重叠/破图、页面风格是否统一、是否存在纯大段文字堆叠，低信息量卡片是否被大容器强行撑满。
...
3. **L3 汇报内容检查**：检查材料是否能支撑汇报。结论要有证据，问题要有归因，方案要有优先级，动作最好有节奏/责任/指标；内容要回应受众关切，避免只讲成果不讲风险、只讲动作不讲原因；同时检查是否存在浓重 AI 味、空泛口号或未解释黑话。
4. **L4 会议室终审**：用“10 秒会议室测试”审视每页。如果一个不了解背景的人在会议室大屏上看 10 秒，是否能知道本页想表达什么、为什么重要、下一步是什么，是否会被黑话或抽象词挡住理解。若答案是否定的，必须重写核心观点条或重组页面。
...
- 生成 HTML 前，若需要更细的版式取舍：读取 `references/layout-pattern-library.md`
